...
class CoursewaresHelper extends AppHelper
{
	...
	function display_for(array $data, array $options = array())
	{
		...
		if (isset($data['Courseware']) && !empty($data['Courseware']))
		{
			$output .= $this->_View->element('Feedback.courseware_index', array('posts' => $data['Courseware']));
		}
	...
	public function form($foreign_model, $foreign_id)
	{
		...
		return $this->_View->element('Feedback.courseware_add', $options);
	}
}